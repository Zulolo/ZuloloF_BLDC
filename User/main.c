...
void initGPIO() {
	...
	// PWM Pin
	GPIO_SetMode(MOSFET_DRV_0_4_PORT,
	MOSFET_DRV_0_PIN | MOSFET_DRV_1_PIN | MOSFET_DRV_2_PIN |
	MOSFET_DRV_3_PIN | MOSFET_DRV_4_PIN, GPIO_PMD_OUTPUT);
	GPIO_SetMode(MOSFET_DRV_5_PORT, MOSFET_DRV_5_PIN, GPIO_PMD_OUTPUT);

	// SPI, I am slave
	GPIO_SetMode(COMM_PORT, COMM_CLK_PIN | COMM_CS_PIN | COMM_RX_PIN,
	GPIO_PMD_INPUT);
	...
	// ACMP Pin
	GPIO_DISABLE_DIGITAL_PATH(ZERO_DETECT_PORT,
			(ZERO_DETECT_A_PIN | ZERO_DETECT_B_PIN | ZERO_DETECT_C_PIN | ZERO_DETECT_M_PIN) << GPIO_OFFD_OFF_SET);
