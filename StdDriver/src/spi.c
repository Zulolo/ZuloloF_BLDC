                   ...
                   uint32_t u32BusClock)
{
    ...
    // assert if is as slave but busclock isn't equal 0
    //
...
/**
  ...
  * @param  u32ActiveLevel specifies the acitve level of slave select signal. (SPI_SS_ACTIVE_HIGH, SPI_SS_ACTIVE_LOW)
  * @return None
  */
...
/**
  ...
  *            (SPI_IE_MASK, SPI_SS_INT_OPT_MASK, SPI_SSTA_INTEN_MASK, SPI_FIFO_TX_INTEN_MASK, 
...
void SPI_EnableInt(SPI_T *spi, uint32_t u32Mask)
{
    ...
    if((u32Mask & SPI_SS_INT_OPT_MASK) == SPI_SS_INT_OPT_MASK)
        spi->CNTRL2 |= SPI_CNTRL2_SS_INT_OPT_Msk;
