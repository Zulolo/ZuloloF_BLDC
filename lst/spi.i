                   ...
                   uint32_t u32BusClock)
{
    ...
    ((CLK_T *) (((uint32_t)0x50000000) + 0x00200))->APBCLK |= (1ul << 12);
    ...
    if(u32BusClock !=0 )
    {
        ...
        if(u32Div > 0xFFFF)
            u32Div = 0xFFFF;
        spi->DIVIDER = (((SPI_T *) (((uint32_t)0x40000000) + 0x30000))->DIVIDER & ~0xffff) | u32Div;
    }
    else
        spi->DIVIDER = 0;
...
void SPI_EnableAutoSS(SPI_T *spi, uint32_t u32SSPinMask, uint32_t u32ActiveLevel)
{
    spi->SSR |= (u32SSPinMask | u32ActiveLevel) | (1ul << 3);
}
...
void SPI_EnableInt(SPI_T *spi, uint32_t u32Mask)
{
    ...
    if((u32Mask & (0x12)) == (0x12))
        spi->FIFO_CTL |= (1ul << 6);
    
    if((u32Mask & (0x14)) == (0x14))
        spi->FIFO_CTL |= (1ul << 21);        
}
