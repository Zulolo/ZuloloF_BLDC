...
#line 86 "User\\BLDCSensorLess.h"
		...
		const uint32_t PHASE_TAB_CLOCKWISE[] = { 
			(1ul << 30) | 0x00000000 | (0x0239ul),
			(1ul << 30) | 0x30000000 | (0x022Dul) | (1ul << 7),
			(1ul << 30) | 0x10000000 | (0x0827ul),
			(1ul << 30) | 0x00000000 | (0x0836ul) | (1ul << 7),
			(1ul << 30) | 0x30000000 | (0x201Eul),
			(1ul << 30) | 0x10000000 | (0x201Bul) | (1ul << 7)
		...
		const uint32_t PHASE_TAB_ANTICLOCKWISE[] = {                        
			(1ul << 30) | 0x00000000 | (0x0239ul) | (1ul << 7),  
			(1ul << 30) | 0x10000000 | (0x201Bul) ,      
			(1ul << 30) | 0x30000000 | (0x201Eul) | (1ul << 7),             
			(1ul << 30) | 0x00000000 | (0x0836ul) , 
			(1ul << 30) | 0x10000000 | (0x0827ul) | (1ul << 7),            
			(1ul << 30) | 0x30000000 | (0x022Dul) 
		};
...
#line 174 "User\\BLDCSensorLess.h"
...
uint16_t canMotorContinueRunning(void)
{
	...
	if ((uint32_t)(iSystemTick - iRotateDetectStartTime) > 200)
	{
		return 0;
	}
	...
	case DETECT_PHASE_1_P:
		if ((uint32_t)(iSystemTick - iStateEnterTime) > 30)
		{
			return (uint16_t)0;
		}
		else
...
void BLDCSensorLessManager(void)
{
	...
	if ((1) == mMotor.structMotor.MSR.MotorPowerOn)
	{
		...
		{
			if ((uint32_t)(iSystemTick - iLastPhaseChangeTime) > 80) 
			{
				stopMotor();
				setError(ERR_INTERNAL);
			}
	...
	{
	case MOTOR_IDLE:
		if (mMotor.structMotor.MCR.MotorNeedToRun && (((iErrorMaster & 0xFFFFFFFEul) == 0) ? (1) : (0)))
		{
	...
	case MOTOR_WAIT_AFTER_LOCATE:
		if (mMotor.structMotor.MCR.MotorNeedToRun && (((iErrorMaster & 0xFFFFFFFEul) == 0) ? (1) : (0)))
		{
			if ((uint32_t)(iSystemTick - iEnterTimeBeforeWait) >= 0)
			{
	...
	case MOTOR_RAMPUP_WO_ZXD:	
		if (mMotor.structMotor.MCR.MotorNeedToRun && (((iErrorMaster & 0xFFFFFFFEul) == 0) ? (1) : (0)))
		{
			...
			if (mMotor.structMotor.ACT_PERIOD <= (1600 - 1))	
			{
				...
				(((ACMP_T *) (((uint32_t)0x40000000) + 0xD0000))->CMPCR[0] |= (1ul << 2) | (1ul << 0));
